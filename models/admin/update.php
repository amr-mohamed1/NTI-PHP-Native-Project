...
if(isset($_GET["id"]) && is_numeric($_GET['id'])){
    $id=$_GET["id"];
$data = all_where("users","id",$_GET['id']);
}
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(empty($_POST["name"]) || empty($_POST["pass"])){
    ...
    update_admin ($name,$email,$sup_hased,$gender,$id);
...
<main>
  <div class="container-fluid">
      ...
      <div class="form-group">
              ...
              <input type="text" value="<?php echo $data[0]["username"]?>" name="name" class="form-control" id="exampleInputName" aria-describedby=""
                  placeholder="Enter Title">
          </div>

          <div class="form-group">
              <label for="exampleInputEmail1">Email</label>
              <input type="email" value="<?php echo $data[0]["email"]?>" name="email" required class="form-control" id="exampleInputName" aria-describedby=""
                  placeholder="Enter Title">
          </div>

          <div class="form-group">
              <label for="exampleInputEmail1">Password</label>
              <input type="password" value="<?php echo $data[0]["password"]?>" name="pass" class="form-control" id="exampleInputName" aria-describedby=""
                  placeholder="Enter Title">
          </div>
          ...
          <label for="gender">Gender</label>
            <select class="custom-select ui search dropdown" name="gender" id="gender" required>
                <option selected disable ddefault  value=""> <?php echo $data[0]["gender"]?></option>
